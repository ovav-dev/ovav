 ...
 */
import { validateF0, validateF5 } from "./f5_integration.js";
...
export async function runOvavValidate(basePath, scope = "all") {
    try {
        switch (scope) {
            case "f0":
...
export async function runOvavDaily(basePath) {
    try {
...
export async function runOvavNextWork(basePath) {
    try {
        const f1 = await validateArchitecture(basePath);
        const f3 = await validateRoles(basePath);
        const recommendations = [];
        if (!f1.valid)
            recommendations.push(`Fix F1 (Architecture): ${f1.errors[0] ?? "unknown"}`);
        if (!f3.valid)
            recommendations.push(`Fix F3 (Roles): ${f3.errors[0] ?? "unknown"}`);
        if (f1.valid && f3.valid)
            recommendations.push("All critical gates pass — system healthy");
        return {
...
export async function runOvavCheckIntegrity(basePath) {
    try {
        const report = await validateF5(basePath);
        return {
