...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    ctx.ui.notify(
      `OVAV MEMORY v3 active — ${ovav.cellCount()} cells loaded`,
      "info"
    );
  });
  ...
  pi.on("turn_end", async (event, ctx) => {
    ...
    if (stats.activeSignals > 0) {
      ctx.ui.setStatus(
        "ovav",
        `OVAV: ${stats.activeSignals} signal(s) · ${stats.cellsInjected} injected`
      );
    } else {
  ...
  pi.registerTool({
    ...
    label: "OVAV Validate",
    description:
      "Run OVAV F0-F5 governance validators on the current project. " +
    ...
    parameters: {
      ...
      properties: {
        scope: {
          ...
          description:
            "Validation scope: all (full F0-F5), or specific gate (f0-f5)",
        },
      },
    },
    async execute(toolCallId, params) {
      const raw = params?.scope as unknown;
      const scope = (typeof raw === "string" && ["all","f0","f1","f2","f3","f4","f5"].includes(raw))
        ? raw as "all" | "f0" | "f1" | "f2" | "f3" | "f4" | "f5"
        : "all";
  ...
  pi.registerTool({
    ...
    description:
      "Returns the next recommended work item from the OVAV decision ledger, " +
  ...
  pi.registerTool({
    ...
    description:
      "Returns a daily governance brief: validator health, open decisions, " +
  ...
  pi.registerTool({
    ...
    description:
      "Checks OVAV system integrity: workspace safety, secrets hygiene, " +
