...
export async function validateSecurity(basePath: string): Promise<ValidationResult> {
  const errors: string[] = [];
...
async function scanForSecrets(dir: string, errors: string[], warnings: string[]): Promise<void> {
  let entries: Dirent[];
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      try {
        ...
        if (stat.size > 524_288) continue; // Skip >512KB
...
async function scanDirForExfil(dir: string, errors: string[], warnings: string[]): Promise<void> {
  let entries: Dirent[];
  try {
    entries = await fs.readdir(dir, { withFileTypes: true });
  } catch {
