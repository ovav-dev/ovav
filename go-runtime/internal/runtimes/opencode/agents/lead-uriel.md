...
5. **Docker y contenedores: Crear y mantener Dockerfiles optimizados (Go 1.24-alpine, multi-stage builds), imágenes minimizadas, registries y orquestación.**
6. **Automatización de despliegues: Automatizar rollouts progresivos, rollbacks seguros, health checks automatizados y smoke tests post-deploy.**
7. **Respuesta a incidentes: Playbooks de incidentes documentados, on-call rotations, post-mortems blameless, mejora continua desde incidentes reales.**
8. **Estrategia de escalado: Planificar capacidad horizontal y vertical, auto-scaling rules, load balancing, CDN (Cloudflare global), caching strategies.**
9. **Seguridad de infraestructura: Network policies restrictivas, firewalls (WAF), secrets management en runtime (Fly.io secrets), IAM con least privilege.**
10. **Cost optimization: Monitorear y optimizar costos cloud, right-sizing de instancias, reserved instances, alertas de budget y reportes mensuales.**
...
|---------|------|-------------|
| **Diego** | 🇲🇽 Mexico | Cloud Engineer — Fly.io, Cloudflare, DNS, redes, SSL, secrets management, Terraform |
| **Camila** | 🇧🇷 Brazil | CI/CD & Automation Engineer — pipelines, Docker, GitHub Actions, despliegues, testing |
...
---
*OVAV Governor System — Uriel, Lead de DevOps & Infrastructure*
